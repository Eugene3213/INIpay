...
/*
	 최초 api 데이터를 로드시에는 객체형 프로그램으로 실행함.
	 JSON 라이브러리를 이용하여 해당 api 배열 데이터를 키값으로 분리 되도록 함.
	 출력 확인 후 Controller로 변경
	 단, 변경 시 주의사항은 객체형 main 메소드를 변경해야 함
	 그 외에 @Autowired, @RequestMapping 이용해서 웹페이지로 샐행 시킨 후 db 입력값을 확인하게 됨.  */
...
public class jasonparser {
	...
	public void parser() {
		...
		try {
			...
			/* 서버에 있는 시간을 로드하여 입력 시 활용하는 query 코드*/
